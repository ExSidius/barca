...
See the [Server API reference](https://barca.dev/reference/server-api/) for the full endpoint reference.
...
Barca is in active development. The core pipeline (parse -> DAG -> plan -> execute -> persist) is working and benchmarked. See the [guide](https://barca.dev/guide/) for a walkthrough.
