"""Timeseries pipeline: 1000 tickers -> normalize -> aggregate -> report.
2002 assets, 2001 edges. Tests massive fan-out, fan-in, large DAG."""
...
def fetch_T0000():
    ...
    return {
        "ticker": "T0000",
        "price": int(h[:8], 16) % 10000 / 100,
        "volume": int(h[8:16], 16) % 1000000,
    }
...
@asset(inputs={"raw": fetch_T0000})
def norm_T0000(raw):
    return {
        "ticker": raw["ticker"],
        "price_norm": raw["price"] / 100,
        "volume_norm": raw["volume"] / 1000000,
    }


@asset(inputs={"raw": fetch_T0001})
def norm_T0001(raw):
    return {
        "ticker": raw["ticker"],
        "price_norm": raw["price"] / 100,
...
def aggregate(**kwargs):
    total_price = sum(v["price_norm"] for v in kwargs.values())
    total_volume = sum(v["volume_norm"] for v in kwargs.values())
    return {
        "tickers": len(kwargs),
        "avg_price": round(total_price / len(kwargs), 4),
        "total_volume": round(total_volume, 4),
    }


@asset(inputs={"agg": aggregate})
def report(agg):
    return {"summary": f"{agg['tickers']} tickers", "status": "complete"}
