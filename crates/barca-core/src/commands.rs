...
fn execute(
    ...
    cost_model.seed(db::load_cost_estimates_sync(&db_path)?);
    ...
    // warm between phases. Uses a multi-thread runtime — the current_thread
    // runtime (used for Turso DB) doesn't drive spawned tasks concurrently
    // enough for the worker I/O task pattern.
    ...
    let mut pool = io_rt
        .block_on(async { crate::io_loop::WorkerPool::start(io_config) })
    ...
    for phase in &exec_plan.phases {
        ...
        let phase_err =
            io_rt.block_on(pool.run_phase(&mut coord, &mut cost_model, Some(on_step_cb)));
    ...
    // All phases done (or aborted) — release the worker pool before persisting.
    pool.shutdown();
    drop(io_rt);
