...
def run_tests():
    ...
    for func in [pure_math, pure_string, pure_with_local_mutation, pure_with_conditional]:
        ...
        results.append((f"Pure: {func.__name__}",
                         r.is_pure,
                         f"is_pure={r.is_pure}, reasons={r.impure_reasons}"))
    ...
    for func, expected_reason in impure_funcs:
        r = analyze_purity(func)
        results.append((f"Impure: {func.__name__} ({expected_reason})",
                         not r.is_pure,
                         f"is_pure={r.is_pure}, reasons={r.impure_reasons[:2]}"))

    # @unsafe functions
    r_unsafe = analyze_purity(acknowledged_impure)
    results.append(("@unsafe: acknowledged_impure",
                     not r_unsafe.is_pure and len(r_unsafe.warnings) == 0,
                     f"is_pure={r_unsafe.is_pure}, warnings={r_unsafe.warnings}, "
                     f"reasons={r_unsafe.impure_reasons}"))
    ...
    results.append(("Warning: getattr produces actionable warning",
                     has_warning,
                     f"warnings={r_getattr.warnings[:1]}"))
    ...
    results.append(("Warning: open() suggests extra_deps",
                     has_file_warning,
                     f"warnings={r_open.warnings[:1]}"))
