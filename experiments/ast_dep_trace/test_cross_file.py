...
from helpers.utils import normalize, format_output
...
def run_tests():
    ...
    results.append(("Case 4: cross-file import (from helpers.utils import normalize)",
                     has_normalize,
                     f"Found normalize: {has_normalize}, "
    ...
    results.append(("Case 4b: cross-file change -> hash changes",
                     h4_orig != h4_mod,
                     f"original={h4_orig[:16]}... modified={h4_mod[:16]}..."))
    ...
    results.append(("Case 7: module attribute call (utils_mod.normalize)",
                     has_utils,
                     f"Found utils module: {has_utils}, "
                     f"deps: {list(deps7.dependencies.keys())}"))
    ...
    results.append(("Case 9: __init__.py re-export",
                     has_exported,
                     f"Found exported_func: {has_exported}, "
                     f"deps: {list(deps9.dependencies.keys())}"))
    ...
    results.append(("Case 15: cross-file named import",
                     has_format,
                     f"Found format_output: {has_format}, "
                     f"deps: {list(deps15.dependencies.keys())}"))
