...
from trace import analyze_purity, clear_caches, extract_dependencies

from unsafe_decorator import is_unsafe, unsafe
...
def run_tests():
    ...
    results.append(("L1: getattr() emits warning", not r1.is_pure and has_warning, f"is_pure={r1.is_pure}, warnings={r1.warnings[:1]}"))

    # L1b: @unsafe silences warning
    r1b = analyze_purity(func_with_getattr_safe)
    results.append(("L1b: @unsafe silences getattr warning", not r1b.is_pure and len(r1b.warnings) == 0, f"is_pure={r1b.is_pure}, warnings={r1b.warnings}"))

    # L2: eval warning
    r2 = analyze_purity(func_with_eval)
    results.append(("L2: eval() detected as impure", not r2.is_pure, f"is_pure={r2.is_pure}, reasons={r2.impure_reasons[:1]}"))

    # L2b: @unsafe silences
    r2b = analyze_purity(func_with_eval_safe)
    results.append(("L2b: @unsafe silences eval warning", not r2b.is_pure and len(r2b.warnings) == 0, f"is_pure={r2b.is_pure}, warnings={r2b.warnings}"))

    # L3: dynamic import warning
    r3 = analyze_purity(func_with_dynamic_import)
    has_import_warning = any("import_module" in w for w in r3.warnings)
    results.append(("L3: importlib.import_module() emits warning", not r3.is_pure and has_import_warning, f"is_pure={r3.is_pure}, warnings={r3.warnings[:1]}"))
    ...
    results.append(
        (
            "L4: @unsafe function detected in dependency cone",
            unsafe_dep_is_marked,
            f"unsafe_source is @unsafe={unsafe_dep_is_marked}, found in deps={has_unsafe_dep}, deps: {list(deps4.dependencies.keys())}",
        )
    )
    ...
    results.append(("L5: open() suggests @asset(extra_deps=...)", not r5.is_pure and has_file_warning, f"is_pure={r5.is_pure}, warnings={r5.warnings[:1]}"))
