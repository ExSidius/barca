...
def run_tests():
    ...
    # Case 1: Same-file function call
    clear_caches()
    h1 = compute_dependency_hash(asset_calls_helper)
    deps1 = extract_dependencies(asset_calls_helper)
    has_helper = any("helper_v1" in k for k in deps1.dependencies)
    results.append(("Case 1: same-file function call",
                     has_helper,
                     f"Found helper_v1 in deps: {has_helper}, "
                     f"deps: {list(deps1.dependencies.keys())}"))
    ...
    results.append(("Case 1b: helper change -> hash changes",
                     hash_changed,
                     f"original={original_hash[:16]}... modified={modified_hash[:16]}..."))

    # Case 2: Global variable
    clear_caches()
    deps2 = extract_dependencies(asset_uses_global)
    has_threshold = any("THRESHOLD" in k for k in deps2.dependencies)
    results.append(("Case 2: global variable reference",
                     has_threshold,
                     f"Found THRESHOLD in deps: {has_threshold}, "
                     f"deps: {list(deps2.dependencies.keys())}"))
    ...
    results.append(("Case 2b: global change -> hash changes",
                     h2_orig != h2_mod,
                     f"original={h2_orig[:16]}... modified={h2_mod[:16]}..."))

    # Case 3: Unrelated function (negative test)
    clear_caches()
    h3_process = compute_dependency_hash(process)
    ...
    results.append(("Case 3: unrelated function NOT in deps",
                     has_clean and not has_unrelated,
                     f"clean={has_clean}, unrelated={has_unrelated}, "
                     f"deps: {list(deps3.dependencies.keys())}"))
    ...
    results.append(("Case 5: transitive chain a->b->c",
                     has_a and has_b,
                     f"chain_a={has_a}, chain_b={has_b}, "
                     f"deps: {list(deps5.dependencies.keys())}"))
    ...
    results.append(("Case 5b: change a -> hash of c changes",
                     h5_orig != h5_mod,
                     f"original={h5_orig[:16]}... modified={h5_mod[:16]}..."))
    ...
    has_closure_dep = any(
        "multiply" in k or "multiplier" in k or "closure:" in k
        for k in deps10.dependencies
    )
    results.append(("Case 10: closure dependency",
                     has_closure_dep,
                     f"Found closure deps: {has_closure_dep}, "
                     f"deps: {list(deps10.dependencies.keys())}"))
    ...
    results.append(("Case 12: comprehension function calls",
                     has_transform and has_validate,
                     f"transform={has_transform}, validate={has_validate}, "
                     f"deps: {list(deps12.dependencies.keys())}"))
    ...
    results.append(("Case 12b: f-string function calls",
                     has_compute,
                     f"Found compute_label: {has_compute}, "
                     f"deps: {list(deps12b.dependencies.keys())}"))
